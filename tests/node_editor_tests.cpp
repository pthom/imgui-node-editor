...
void NodeEditorTests_ShowGui()
{
    ...
    ImGui::SetNextWindowSize(ImVec2(em * 38.0f, em * 35.0f), ImGuiCond_FirstUseEver);
...
static void SetZoom(ImGuiTestContext* ctx, float zoom)
{
    for (int i = 0; i < 4 && ImAbs(gScene.Zoom - zoom) > 0.01f; i++)
    {
        ...
        // The zoom is animated over a fraction of a second: wait until it did not change for 10 frames.
        ...
        for (int stable_frames = 0; stable_frames < 10 && ImGui::GetTime() - start_time < 3.0; )
        {
...
// Pan until a recorded item lies in the upper left part of the canvas (which leaves room for a popup below and on its right)
static void BringIntoView(ImGuiTestContext* ctx, const char* name)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        const ImRect target(gScene.CanvasRect.Min + ImVec2(em, em), gScene.CanvasRect.Min + gScene.CanvasRect.GetSize() * 0.6f);
...
// Run a check at zoom 1 / 0.5 / 2, then at zoom 1 after a pan. The draw list is checked over all the frames of the run.
...
static void RunInAllViews(ImGuiTestContext* ctx, ViewCheck check)
{
    ...
    const float zooms[] = { 1.0f, 0.5f, 2.0f };
    for (float zoom : zooms)
    {
        SetZoom(ctx, zoom);
        if (ctx->IsError())
            return;
        check(ctx, Str16f("zoom_%.1f", zoom).c_str());
        ctx->PopupCloseAll();
        CheckDrawList(ctx);
        if (ctx->IsError())
            return;
